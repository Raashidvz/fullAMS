...
if (isset($_POST['add_event'])) {
    ...
    if (!empty($eventDate) && !empty($eventTitle)) {
        ...
        if ($dbcon) {
            ...
            if ($stmt) {
                $stmt->bind_param("sss",$_SESSION['STUDENT_ID'], $eventDate, $eventTitle);
                if ($stmt->execute()) {
...
$query = "SELECT * FROM events WHERE TEACHER_ID='$_SESSION[STUDENT_ID]' AND event_date BETWEEN '$startOfMonth' AND '$endOfMonth'";
